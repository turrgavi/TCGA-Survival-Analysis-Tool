...
check_stage = FALSE
check_stage_break = FALSE #if true, check_stage must also be true
...
if (check_stage == TRUE){
  survival_data[,"Quartile"] <- as.character(clinical_data_all[,"clinical_stage"])
...
i = 2
for (i in 1:length(survival_data[,1])){
  if (survival_data[i,"Status"] == "Alive"){
    survival_data[i,"Status"] <- 0
    survival_data[i,"Event"] <- survival_cutoff
  }
  if (survival_data[i,"Status"] == "Dead"){
    survival_data[i,"Status"] <- 1
