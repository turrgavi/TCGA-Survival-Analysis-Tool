...
#This can be optimized by using the merge function
...
#  p=p+1
#  print(p)
#  if (str_sub(test_buffer[1,p], -3) != "-01A" & str_sub(test_buffer[1,p], -3) != "-01B")
#  {
...
for (i in 1:run_time_quart){
  ...
  LQ <- round(quantile(as.numeric(test_buffer[p,2:length(test_buffer[2,])]), 0.25), 2)
  UQ <- round(quantile(as.numeric(test_buffer[p,2:length(test_buffer[2,])]), 0.75), 2)
...
clinical_data_all <- read.csv("TCGAOV_clinical_raw.csv", header = TRUE)
